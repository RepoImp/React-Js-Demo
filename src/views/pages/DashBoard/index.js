...
export default function Dashboard() {
    ...
    const handleClose = (setCustomer, setRemark, setFtpDestinationPath) => {
        setOpen(false);
    ...
    React.useEffect(() => {
        ...
        if (search) {
            setGmtCvsfileLists((gmtCvsfileLists) =>
                gmtCvsfileLists.filter((gmtCvs) => {
    ...
    const onUploadHandler = async (customer, setCustomer, remark, setRemark, ftpDestinationPath, setFtpDestinationPath) => {
        setIsCheckValid(true);
        if (customer !== null && fileName !== '' && ftpDestinationPath !== null) {
            setLoading(true);
            let params = {
                file: file,
            ...
            if (data.status === 200 && data?.data?.status === 200) {
                setLoading(false);
                setIsCheckValid(false);
                handleClose(setCustomer, setRemark, setFtpDestinationPath);
                refetch();
